...
void readStudent(student plist) {
    printf("    %s %d %s교수님 %s %d 총벌점: %d점 상담신청 여부: %c\n", plist.name, plist.studentId, plist.teamProf, plist.rcName, plist.roomNum, plist.totalNum, plist.apply);
}
...
int selectno(student *s,int count){
    int no;
    listStudent(s,count);
    printf("숫자를 고르시오(취소=0): ");
    scanf("%d",&no);
    return no;
}
